...
export default (viewport: Viewport, newScroll: Position): Viewport => {
  ...
  const position: ClientRect = viewport.elementPosition;
  const top: number = position.top + newScroll.y;
  const left: number = position.left + newScroll.x;
  ...
  const updated: Viewport = {
    ...
    elementPosition: viewport.elementPosition,
  };
